...
1. Gather the following from the user (ask for anything not already provided):
   ...
   - **Blockchain**: Which chain for payment (Base, Ethereum, Optimism, or Arbitrum)
   - **Time limit**: How many hours the worker has to complete the task
   - **Payment type** (optional): USDC (default) or pyUSD. If they choose pyUSD, workers can accept payment via **PayPal or Venmo** without needing a crypto wallet.

2. Confirm the details with the user before posting.

3. Call `post_job` with the confirmed parameters.

4. Show the user the created job ID and let them know humans can now apply. Suggest using `/mm-myjobs` to monitor applicants.
