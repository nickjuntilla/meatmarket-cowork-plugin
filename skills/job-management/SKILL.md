...
- **Automated check**: Run `/meatmarket-check` to have the AI poll for new activity, evaluate applicants and proofs, and take action based on your settings.
...
- **Record payment**: After sending crypto on-chain, call `mark_payment_sent` with the job ID and transaction URL. Or if auto-pay is enabled, the `/meatmarket-check` pipeline handles this automatically after verifying the proof.
